(function () {
  ...
  async function load() {
    ...
    const { data, error } = await requireClient()
      .from('orders')
      .select('id, order_code, customer_id, status, operational_status, subtotal, discount, shipping_fee, total, customer_notes, submitted_at, created_at, payment_validated_at, approved_at, capacity_confirmed_at, production_due_at, delivery_method, delivery_details, tracking_code, refund_amount, refund_requested_at, refund_processed_at, order_items(id, product_id, product_snapshot, quantity, configuration, unit_price, line_total), order_operational_history(id, from_status, to_status, changed_by, reason, metadata, created_at)')
  ...
  function canTransitionTo(status) {
    ...
    if (['payment_received', 'refunded'].includes(status)) return false;
